...
/* regex entreprise et message autorisent quasiment tous les caractères :
...
pour incrémenter j de regex qui doit correspodnre au i du champ (idem pour k de l'erreur)  */
...
function spanChampsVides (champ) {
    ...
    if (champ.value ==="") {
        ...
        champ.parentNode.querySelector(".champs-obligatoires").appendChild(span);
        champ.parentNode.classList.add("erreur");
    }
};
...
function removeSpanVideRegex (champ, regex) {
    ...
    if (champ.value !== "") {
        let resultat = regex.test(champ.value);
        if (resultat) {   
            const span = champ.parentNode.querySelector("span");
            if (span) {                                                         // prévoit le cas où il n'y a pas de span d'erreur, évite un bug
              span.remove();                                    
              champ.parentNode.classList.remove("erreur");
        }}}
})};
